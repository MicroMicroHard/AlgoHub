...
import DataStructure.heapStackQueue.stack.monotonousStack.MonotonousStack;
...
public class MonotonousStackTest {
    ...
    @Test
    public void TestGetNextBiggerValue() {
        test(new MonotonousStack());
    }

    public void test(MonotonousStack ms) {
        int[] answer = ms.GetNextBiggerValue(source01);
        Assert.assertArrayEquals(answer, target01);
        answer = ms.GetNextBiggerValue(source02);
        Assert.assertArrayEquals(answer, target02);
        answer = ms.GetNextBiggerValue(source03);
        Assert.assertArrayEquals(answer, target03);
        answer = ms.GetNextBiggerValue(source04);
        Assert.assertArrayEquals(answer, target04);
        answer = ms.GetNextBiggerValue(source05);
        Assert.assertArrayEquals(answer, target05);
        answer = ms.GetNextBiggerValue(source06);
        Assert.assertArrayEquals(answer, target06);
        answer = ms.GetNextBiggerValue(source07);
        Assert.assertArrayEquals(answer, target07);
    }

    //@Test
    public void testcycilmethod(MonotonousStack ms) {
