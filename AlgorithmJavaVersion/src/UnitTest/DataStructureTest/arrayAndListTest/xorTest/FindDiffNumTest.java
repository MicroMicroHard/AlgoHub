...
import DataStructure.arrayRelevant.xor.FindDiffNum;
...
public class FindDiffNumTest {
    ...
    public void testfindRepertNum() {
        int target = findDiffNum.findRepeatNum(demoRepert_01);
        assert target == Integer.MIN_VALUE;
        target = findDiffNum.findRepeatNum(demoRepert_02);
        assert target == Integer.MIN_VALUE;
        target = findDiffNum.findRepeatNum(demoRepert_03);
        assert target == 0;
        target = findDiffNum.findRepeatNum(demoRepert_04);
        assert target == 10;
        target = findDiffNum.findRepeatNum(demoRepert_05);
        assert target == 11;
        target = findDiffNum.findRepeatNum(demoRepert_06);
        assert target == 9;
    }
