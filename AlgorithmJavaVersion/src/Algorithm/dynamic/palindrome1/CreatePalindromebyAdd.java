 ...
 *@version 1.0
 */
public class CreatePalindromebyAdd extends Palindrome{
	...
	public int headAndTailCompare(String testString) {
		...
		for (int head = 0, tail = length - 1; head < tail; head++, tail--) {
			if(testString.charAt(head) != testString.charAt(tail)){
				//如果是头指针不相等
				if(JudgePalindrome(testString.substring(head, tail))){
					return tail;
				}else if(JudgePalindrome(testString.substring(head + 1, tail+ 1))){
					return head;
				}else {
					return -1;
				}
	...
	//方法二：删除构造
	public int CreatePalindromeByAdd(String testString) {
		...
		for (int i = 0; i < testString.length(); i++) {
			...
			if(JudgePalindrome(SourceString)){
				return i;
			}
		}
		return -1;
